...
class HelloNear {
  ...
  @view({}) // This method is read-only and can be called for free
  get_skills({ addr }: { addr: string }): Skill[] {
  ...
  @view({}) // This method is read-only and can be called for free
  get_mentor_bookings({ addr }: { addr: string }): Booking[] {
    return this.bookings.filter((b) => b.mentor_addr === addr);
  }

  @view({}) // This method is read-only and can be called for free
  get_mentee_bookings({ addr }: { addr: string }): Booking[] {
